...
def canonicalize_job_url(url: Any) -> str:
    ...
    if "asml.com" in netloc or "appliedmaterials.com" in netloc or "lamresearch.com" in netloc or "careers.amd.com" in netloc:
...
def looks_like_internship(*texts: Any) -> bool:
    corpus = normalize_text_for_match(" ".join(clean_text(t) for t in texts if clean_text(t)))
    intern_terms = ["intern", "internship", "apprentice", "co-op", "thesis internship", "인턴"]
    return any(contains_term(corpus, tok) for tok in intern_terms)
...
def explicit_company_match(text: Any, aliases: list[str]) -> bool:
    t = clean_text(text).lower()
    return any(contains_term(t, alias.lower()) or alias.lower() in t for alias in aliases)
