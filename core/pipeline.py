...
def reconcile_records(sheet_key: str, incoming_records: list[JobRecord], state_manager, today_str: str, miss_threshold: int, *, source_scope: set[str] | None = None):
    ...
    for record in incoming_records:
        if not is_valid_record(record):
            rows.append({"company": sheet_key, "unique_key": record.unique_key, "source": record.source, "title": record.title, "url": record.url, "decision": "DROP", "reason": "invalid_incoming_record"})
            continue
        ...
        rows.append({"company": sheet_key, "unique_key": key, "source": record.source, "title": record.title, "url": record.url, "decision": "ACTIVE", "reason": "seen_in_run"})
    ...
    for key, payload in list(state.items()):
        ...
        if not is_valid_record_payload(payload):
            state.pop(key, None)
            rows.append({"company": sheet_key, "unique_key": key, "source": payload.get("source", ""), "title": payload.get("title", ""), "url": payload.get("url", ""), "decision": "DROP", "reason": "invalid_state_payload"})
            continue
        payload_source = payload.get("source", "")
        if source_scope and payload_source not in source_scope:
            active.append(_record_from_payload(sheet_key, payload))
            rows.append({"company": sheet_key, "unique_key": key, "source": payload_source, "title": payload.get("title", ""), "url": payload.get("url", ""), "decision": "KEEP", "reason": "out_of_scope_preserved"})
            continue
        ...
        if deadline_passed_with_grace(deadline, today_str) or miss_count >= miss_threshold:
            ...
            state.pop(key, None)
            rows.append({"company": sheet_key, "unique_key": key, "source": payload_source, "title": payload.get("title", ""), "url": payload.get("url", ""), "decision": "CLOSE", "reason": "deadline_or_miss_threshold"})
            continue
        active.append(_record_from_payload(sheet_key, payload))
        rows.append({"company": sheet_key, "unique_key": key, "source": payload_source, "title": payload.get("title", ""), "url": payload.get("url", ""), "decision": "KEEP", "reason": f"miss_count={miss_count}"})

    fieldnames = ["company", "unique_key", "source", "title", "url", "decision", "reason"]
    write_csv_rows(f"reconcile_{safe_token(sheet_key)}.csv", fieldnames, rows, append=False)
    append_audit_rows("reconcile", rows)
