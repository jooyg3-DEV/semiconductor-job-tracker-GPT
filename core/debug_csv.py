...
SUMMARY_FIELDS = [
    "timestamp", "kind", "company", "source", "source_type", "platform", "fetched", "filtered", "written", "closed", "decision", "reason", "note",
]


def append_summary_row(**payload) -> Path:
    row = {k: "" for k in SUMMARY_FIELDS}
    row["timestamp"] = payload.get("timestamp") or datetime.utcnow().isoformat(timespec="seconds")
    for k in row:
        if k in payload:
            row[k] = payload.get(k, "")
    return write_csv_rows("summary.csv", SUMMARY_FIELDS, [row], append=True)
