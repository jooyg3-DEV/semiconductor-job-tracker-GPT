...
from core.utils import contains_term, find_matches, has_experience, has_masters, has_phd, looks_like_internship, normalize_text_for_match, shorten_cell, summarize_requirements
...
HARD_EXCLUDE_TERMS = [
    "intern", "internship", "apprentice", "co-op", "인턴",
...
def evaluate_record(company_name: str, record: JobRecord, source_type: str, include_keywords: list[str], exclude_keywords: list[str]):
    ...
    internship_flag = looks_like_internship(record.title, record.employment_type, record.qualification, record.metadata.get("summary", ""), record.raw_text)
    ...
    if internship_flag and "internship" not in hard_excludes:
...
def filter_records(company_name: str, source_name: str, source_type: str, records: list[JobRecord], include_keywords: list[str], exclude_keywords: list[str]) -> list[JobRecord]:
    ...
    write_csv_rows(
        f"filter_{safe_token(company_name)}_{safe_token(source_name)}.csv",
        ["company", "source", "source_type", "title", "url", "canonical_url", "job_function", "location", "employment_type", "matched_keyword", "score", "include_matches", "exclude_matches", "hard_excludes", "decision", "reason"],
        rows,
        append=False,
    )
