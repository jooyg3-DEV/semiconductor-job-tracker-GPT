...
class GoogleSheetsClient:
    ...
    def _split_valid_records(self, sheet_title: str, records: list[JobRecord]) -> tuple[list[JobRecord], list[JobRecord]]:
        ...
        prewrite_rows = [{
                ...
                "company": r.company,
                "source": r.source,
                "deadline": r.deadline or "없음",
                "title": r.title,
                "qualification": r.qualification,
                "job_function": r.job_function,
                "url": r.url,
                "canonical_url": r.canonical_url,
                "location": r.location or "미기재",
                "employment_type": r.employment_type,
                "recruitment_type": r.recruitment_type,
                "experience_flag": r.experience_flag,
                "masters_flag": r.masters_flag,
                "phd_flag": r.phd_flag,
                "is_valid_row": "Y" if is_valid_record(r) else "N",
            } for r in records]
        write_csv_rows(
            f"prewrite_{safe_token(sheet_title)}.csv",
            ["search_date", "company", "source", "deadline", "title", "qualification", "job_function", "url", "canonical_url", "location", "employment_type", "recruitment_type", "experience_flag", "masters_flag", "phd_flag", "is_valid_row"],
            prewrite_rows,
            append=False,
        )
    ...
    def _append_rows(self, ws, rows: list[list[str]]) -> None:
        if not rows:
            return
        sanitized = [[self._sanitize_cell(v) for v in row] for row in rows]
        self._with_retry(lambda: ws.append_rows(sanitized, value_input_option="RAW", insert_data_option="INSERT_ROWS"))

    def write_company_records(self, sheet_title: str, records: list[JobRecord], *, preserve_existing_order: bool = False, append_only: bool = True, write_to_sheet: bool = True) -> None:
        ws = self._ensure_worksheet(sheet_title, create_if_missing=False)
        if ws is None:
            print(f"[WARN] company worksheet missing, skipped: {sheet_title}")
            return
        today_str = datetime.now().strftime("%Y-%m-%d")
        if preserve_existing_order and not append_only:
        ...
        rows_to_write = [r.to_row(today_str) for r in records]
        if not write_to_sheet:
            return
        existing = self._with_retry(lambda: ws.get_all_values())
        if not existing:
            self._replace_sheet_values(ws, [HEADERS], clear_first=True)
        elif existing and existing[0][:len(HEADERS)] != HEADERS:
            self._replace_sheet_values(ws, [HEADERS] + existing[1:], clear_first=True)
        self._append_rows(ws, rows_to_write)

    def write_closed_records(self, records: list[JobRecord], *, write_to_sheet: bool = True) -> None:
        ...
        rows_to_write = [r.to_row(today_str) for r in records]
        if not write_to_sheet or not rows_to_write:
            return
        existing = self._with_retry(lambda: ws.get_all_values())
        if not existing:
            self._replace_sheet_values(ws, [HEADERS], clear_first=True)
    ...
    def append_company_csv_rows(self, sheet_title: str, rows: list[dict[str, str]]) -> None:
        ws = self._ensure_worksheet(sheet_title, create_if_missing=True)
        if ws is None:
            return
        existing = self._with_retry(lambda: ws.get_all_values())
        if not existing:
        ...
        for row in rows:
            values.append([
                row.get("search_date") or row.get("검색일") or datetime.now().strftime("%Y-%m-%d"),
                row.get("source") or row.get("출처", ""),
                row.get("deadline") or row.get("마감일", "없음"),
                row.get("company") or row.get("회사", sheet_title),
                row.get("title") or row.get("공고명", ""),
                row.get("qualification") or row.get("지원자격", ""),
                row.get("job_function") or row.get("채용직무", ""),
                row.get("location") or row.get("근무지", "미기재"),
                row.get("employment_type") or row.get("채용형태", ""),
                row.get("recruitment_type") or row.get("모집구분", "일반"),
                row.get("experience_flag") or row.get("경력", "N"),
                row.get("masters_flag") or row.get("석사", "N"),
                row.get("phd_flag") or row.get("박사", "N"),
                row.get("url") or row.get("링크", ""),
            ])
        self._append_rows(ws, values)
