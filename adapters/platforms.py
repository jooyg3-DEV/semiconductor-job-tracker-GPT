...
from core.search_plan import LINKEDIN_COMPANY_SLUGS, get_platform_search_terms
...
ALIASES = {
    ...
    "Applied Materials": [
        ...
        "어플라이드 머티어리얼즈", "어플라이드 머티어리얼즈 코리아", "어플라이드머티어리얼즈", "어플라이드머티어리얼즈코리아", "amat", "어플라이드 코리아",
    ],
...
def _extract_company_from_soup(platform: str, soup: BeautifulSoup, title: str) -> tuple[str, str]:
    selectors = {
        "사람인": [".company_nm", ".recruit_company_name", ".company a", ".company_info .name", ".recruit_info .company_name", "meta[property='og:site_name']", "meta[property='og:title']"],
        "잡코리아": [".coName", ".tplJobView .tit_company", ".company .name", ".companyName", "meta[property='og:title']", "meta[property='og:description']"],
...
class SearchPlatformAdapter(BaseAdapter):
        ...
        if platform == "링크드인":
            ...
            for keyword in search_terms:
                ...
                if key in seen:
                    continue
                seen.add(key)
                pairs.append((keyword, "", build_url(company_name, keyword, "")))
            return pairs
        company_queries = get_platform_search_terms(company_name)
        if self.light_mode:
            company_queries = company_queries[:2]
    ...
    def _detail_payload(self, platform: str, url: str) -> dict[str, str]:
        ...
        if not qualification:
            qualification = clean_text(raw[:500])
        return {
            "title": title,
            "raw": raw,
            "final_url": final_url,
            ...
            "company_name_raw": detail_company,
        }
    ...
    def _company_alias_match(self, company: str, extracted_company: str, title: str, raw: str, url: str) -> tuple[bool, str]:
        ...
        if extracted_company and explicit_company_match(extracted_company, aliases):
            return True, extracted_company
        whole = f"{extracted_company} {title} {raw[:1200]}"
        fallback_ok = explicit_company_match(whole, aliases)
        if fallback_ok:
            return True, extracted_company or clean_text(title)
        return False, extracted_company
    ...
    def fetch(self):
        ...
        for target_company in target_companies:
            ...
            for keyword, region_hint, title, url, canonical, source_search_url in accepted_candidates:
                ...
                record.metadata["company_name_raw"] = payload.get("company_name_raw", "")
