...
from core.utils import clean_text, stringify
...
class TSMCAdapter(BaseAdapter):
    def fetch(self):
        ...
        for url in job_links[:80]:
            ...
            title = clean_text(stringify(data.get('title')) or (soup.find('h1').get_text(' ', strip=True) if soup.find('h1') else ''))
            if not title:
                continue
            ...
            if isinstance(jl, dict):
                ...
                location = clean_text(stringify(addr.get('streetAddress')) or stringify(addr.get('addressLocality')) or stringify(addr.get('addressCountry')))
            elif isinstance(jl, list):
                location = clean_text(stringify(jl))
            employment = clean_text(stringify(data.get('employmentType')))
            qualification = clean_text(stringify(data.get('educationRequirements')))
            exp = clean_text(stringify(data.get('experienceRequirements')))
            quals = clean_text(stringify(data.get('qualifications')))
            summary = ' / '.join([x for x in [qualification, exp, quals] if x])
